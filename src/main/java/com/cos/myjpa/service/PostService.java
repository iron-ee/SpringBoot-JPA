...
public class PostService {
	...
	public Post 글쓰기(PostSaveReqDto postSaveReqDto, User principal) {	// title, content
		...
		post.setUser(principal);	// FK를 insert 할 수 있음.
	...
	public Post 수정하기(Long id, PostUpdateReqDto postUpdateReqDto) {
		...
		return postEntity;
	}	// 서비스(트랜잭션) 종료시 영속성 컨텍스트에 영속화 되어있는 모든 객체의 변경을 감지하여 변경된 아이들을 flush 한다. (commit) = 더티체킹
		// JPA 영속성 컨텍스트는 변경감지를 하는데 언제하느냐? 서비스 종료시에 함 !!
	
	@Transactional
	public void 삭제하기(Long id) {	// 삭제는 return 이 필요없다. 삭제하다가 오류나서 GlobalException으로 처리하면 됨.
